 ...
 */
switch ($accion) {
    case 'InventarioTabla':
        try {
            InventarioTabla(filter_input(INPUT_POST, "query"));
        } catch (Exception $exc) {
...
function InventarioTabla($query) {
    $Inventario = new InventarioDistrito();
    /**
     * Regresa los registros de los estados
     */
    echo json_encode($Inventario->InventarioTabla($query));
}