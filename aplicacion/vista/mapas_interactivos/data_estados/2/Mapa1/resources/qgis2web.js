...
map.getView().fit([-13755962.559409, 2970620.244890, -12085170.730910, 4008572.700758], map.getSize());
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") == undefined)) {
            ...
            if (typeof clusteredFeatures !== "undefined") {
                ...
                currentFeatureKeys = currentFeature.getKeys();
                if (doPopup) {
                    const keysNotPermited = ['cvegeo', 'cov_', 'cov_id', 'entmun', 'auxiliary_storage_labeling_positionx', 'auxiliary_storage_labeling_positiony', 'objectid', 'id_edo', 'nom_edo', 'shape_len', 'pid', 'entmun2', 'count_entm', 'sum_sup_m2', 'dip'];
                    popupText += '<li><table>';
                    // debugger;
                    for (var i=0; i<currentFeatureKeys.length; i++) {
                        const busqueda = keysNotPermited.find(element => element === currentFeatureKeys[i].toLowerCase());
                        if (currentFeatureKeys[i] != 'geometry' && !busqueda && !popupText.includes(currentFeatureKeys[i]) && currentFeature.get(currentFeatureKeys[i])!=null) {
                            popupField = '';
                            if (layer.get('fieldLabels')[currentFeatureKeys[i]] == "inline label") {
                                popupField += '<th>' + layer.get('fieldAliases')[currentFeatureKeys[i]] + ':</th><td>';
                            } else {
                            ...
                            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
                                const aaa = currentFeature.get(currentFeatureKeys[i]);
                                ...
                                popupField += '<b>' + currentFeatureKeys[i] + ' </b>: ' +(currentFeature.get(currentFeatureKeys[i]) != null ? '<img src="images/' + currentFeature.get(currentFeatureKeys[i]).replace(/[\\\/:]/g, '_').trim()  + '" /></td>' : '');
                            }
