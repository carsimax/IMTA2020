...
<div class="col-sm-12 pt-3 pb-2 mb-3 border-bottom">
    <h4>Marginación por municipio</h4>
</div>
<div class="row">
    <div class="col-sm">
        <a href="municipiomarginacion/nuevo.php" class="btn btn-gob btn-fill  btn-block"><i class="fas fa-plus"></i> Nuevo Registro</a>
    </div>
    <div class="col-sm">
        <a href="municipiomarginacion/excel.php" class="btn btn-gob btn-fill btn-block"><i class="fas fa-file-pdf"></i> Importar desde Excel</a>
    </div>
</div>
<hr>
...
<script>
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            $.each(JSON.parse(resp), function (index, item) {
                data.push([item.id_registro_municipio, item.estado, item.municipio, item.analf, item.sprim, item.im, item.gm, item.pob_tot, item.anio])
            });
    ...
    $('#example').on('click', '#editar', function () {
       ...
       $.redirect("/aplicacion/vista/crud/municipiomarginacion/editar.php", {ID: data[0]}, "POST");
    });
