...
<div class="col-sm-12 pt-3 pb-2 mb-3 border-bottom">
    <h4>Estaciones Hidrométricas</h4>
</div>
<div class="row">
    <div class="col-sm">
        <a href="estacioneshidrometricas/nuevo.php" class="btn btn-gob btn-fill  btn-block"><i class="fas fa-plus"></i> Nueva Estación</a>
    </div>
    <div class="col-sm">
        <a href="estacioneshidrometricas/excel.php" class="btn btn-gob btn-fill btn-block"><i class="fas fa-file-pdf"></i> Importar desde Excel</a>
    </div>
</div>
<hr>
...
<script>
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            $.each(JSON.parse(resp), function (index, item) {
                data.push([item.id_estacion_hidrometrica, item.nombre, item.corriente, item.cuenca, item.estado]);
            });
    ...
    $('#example').on('click', '#editar', function () {
        ...
        $.redirect("/aplicacion/vista/crud/estacioneshidrometricas/editar.php", {ID: data[0]}, "POST");
    });
                    ...
                    success: function (response) {
                        if (response == 1) {
                            Swal.fire({
                                ...
                                $.redirect("/aplicacion/vista/crud/dbadmin.php", {tablaP: "EstacionesHidrometricas"}, "POST");
                            });
                        }
                    },
                });
